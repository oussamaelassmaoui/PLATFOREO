...
@section('content')
    <form class="row g-3 needs-validation" action="{{ route('Articles.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="col-md-6">
            ...
            <div class="position-relative">
                <input type="text" name="slug" class="form-control h-58 @error('slug') is-invalid @enderror"
                    id="validationCustom11" value="{{ old('slug') }}" required>
        ...
        <div class="col-md-12">
            ...
            <div class="position-relative">
                <textarea cols="30" rows="5" name="text1" class="form-control py-3 @error('text1') is-invalid @enderror"
                    id="text1" placeholder="Notes" required>{{ old('text1') }}</textarea>
        ...
        <div class="col-md-12">
            <label for="validationCustom18" class="form-label label">text motivation :</label>
            <div class="position-relative">
                <textarea cols="30" rows="5" name="text_motivation"
                    class="form-control py-3 @error('text_motivation') is-invalid @enderror" id="motivation" placeholder="Notes"
                    required>{{ old('text_motivation') }}</textarea>

                @error('text_motivation')
                    <div class="text-danger">{{ $message }} </div>
                @enderror
            </div>
        </div>
    ...
    <script>
        ...
        $('#motivation').summernote({
            placeholder: 'text motivation',
            tabsize: 2,
            height: 100
        });
    </script>
